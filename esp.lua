...
if not Drawing or not Drawing.new or not Drawing.Fonts then
	loadstring(game.HttpGet(game, "https://pastebin.com/raw/ixgU7g92"))()
...
local ConfigLibrary = loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/Captive/Config-Library/main/Main.lua"))()
...
if identifyexecutor() == "Solara" then -- Quads are broken on Solara.
	local DrawQuad = loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/Captive/Custom-Quad-Render-Object/main/Main.lua"))() -- Custom Quad Drawing Object
...
if not Degrade then
	...
	local DrawQuad = loadstring(game.HttpGet(game, "https://raw.githubusercontent.com/ketty463/v2/refs/heads/main/main.lua"))() -- Custom Quad Drawing Object
	local _Drawingnew = clonefunction(Drawing.new)
	...
	warn("Captive_ESP > Your exploit does not support this module's optimizations! The visuals might be laggy and decrease performance.")
end
]=]
...
if CaptiveDeveloperESP and CaptiveDeveloperESP.Exit then
	CaptiveDeveloperESP:Exit()
end

--// Settings

getgenv().CaptiveDeveloperESP = {
	DeveloperSettings = {
		Path = "Captive Developer/Captive ESP/Configuration.cfg",
...
local Environment = getgenv().CaptiveDeveloperESP
...
local CoreFunctions = {
	...
	GenerateHash = function(Bits)
		...
		for _ = 1, Bits do
			Result ..= ("Captive_ESP")[mathrandom(1, 2) == 1 and "upper" or "lower"](stringchar(mathrandom(97, 122)))
		end
...
local UtilityFunctions = {
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		assert(self, "Captive_ESP > UtilityFunctions.WrapObject - Internal error, unassigned parameter \"self\".")
		...
		if not Entry.IsAPlayer then
			if not pcall(function()
				...
				warn("Captive_ESP > UtilityFunctions.WrapObject - Attempted to wrap object of an unsupported class type: \""..(__index(Entry.Object, "ClassName") or "N / A").."\"")
...
Environment.UnwrapAll = function(self) -- METHOD | (<void>) => <void>
	assert(self, "Captive_ESP.UnwrapAll: Missing parameter #1 \"self\" <table>.")
...
Environment.Restart = function(self) -- METHOD | (<void>) => <void>
	assert(self, "Captive_ESP.Restart: Missing parameter #1 \"self\" <table>.")
...
Environment.Exit = function(self) -- METHOD | (<void>) => <void>
	assert(self, "Captive_ESP.Exit: Missing parameter #1 \"self\" <table>.")
	...
	if self:UnwrapAll() then
		...
		for Index, _ in next, Environment do
			getgenv().CaptiveDeveloperESP[Index] = nil
		end
		...
		getgenv().CaptiveDeveloperESP = nil
	end
end
...
Environment.UpdateConfiguration = function(DeveloperSettings, Settings, Properties) -- (<table> DeveloperSettings, <table> Settings, <table> Properties) => <table> New Environment
	assert(DeveloperSettings, "Captive_ESP.UpdateConfiguration: Missing parameter #1 \"DeveloperSettings\" <table>.")
	assert(Settings, "Captive_ESP.UpdateConfiguration: Missing parameter #2 \"Settings\" <table>.")
	assert(Properties, "Captive_ESP.UpdateConfiguration: Missing parameter #3 \"Properties\" <table>.")

	getgenv().CaptiveDeveloperESP.DeveloperSettings = DeveloperSettings
	getgenv().CaptiveDeveloperESP.Settings = Settings
	getgenv().CaptiveDeveloperESP.Properties = Properties

	Environment = getgenv().CaptiveDeveloperESP

	return Environment
end

Environment.LoadConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "Captive_ESP.LoadConfiguration: Missing parameter #1 \"self\" <table>.")
...
Environment.SaveConfiguration = function(self) -- METHOD | (<void>) => <void>
	assert(self, "Captive_ESP.SaveConfiguration: Missing parameter #1 \"self\" <table>.")
